...
from tests.t_infrastructure.auth.factory.password import PasswordLoginInitDataDTOFactory, PasswordLoginDTOFactory
...
class BaseClientTest(ABC):
    ...
    @staticmethod
    def _get_auth_telegram_provider_data(valid=True) -> TelegramProviderDataDTO:
    ...
    def client(self) -> TestClient:
        def get_extended_auth_router():
            original_auth = import_module("interfaces.fastapi.routers.auth")
            ...
            async def create_super_user():
                ...
                return {"account": superuser.to_dict(), "password": "passw"}

            return router

        fake_auth_module = types.ModuleType("auth")
        fake_auth_module.router = get_extended_auth_router()
        with patch.dict("sys.modules", {"interfaces.fastapi.routers.auth": fake_auth_module}):
            import interfaces.fastapi.main as main_module
            reload(main_module)
    ...
    def _get_superuser_token(self, client):
        ...
        login_dto = PasswordLoginDTOFactory(
    ...
    def _create_languages(self, client, limit=10):
