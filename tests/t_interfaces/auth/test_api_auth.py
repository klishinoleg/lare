...
class TestApiAuth(BaseClientTest):
    ...
    async def test_telegram_auth_and_me_data(self, client: TestClient):
        ...
        me_response = client.get(self.route_me, headers={"Authorization": f"Bearer {token}"})
        assert me_response.status_code == 200
        me_data = me_response.json()
        assert me_data["id"] == account["id"]
        profiles_response = client.get(self.route_profiles, headers={"Authorization": f"Bearer {token}"})
    ...
    async def test_password_register_login_me_change_password(self, client: TestClient):
        ...
        login_response = client.post(self.route_auth_login, json=login_init_dto.model_dump())
        ...
        new_login_response = client.post(self.route_auth_login, json=new_login_dto.model_dump())
